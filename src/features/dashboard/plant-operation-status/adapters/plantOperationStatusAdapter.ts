...
function formatTriple(values: NumberLike[], unit = '') {
  const formattedValues = values.map((value) => formatNumber(value));
  const result = formattedValues.join('/');

  return unit && !formattedValues.every((value) => value === EMPTY_VALUE) ? `${result}${unit}` : result;
}
...
function createPowerRows(source: GridStatusResponseDto | EssStatusResponseDto | PcsStatusResponseDto | DieselStatusResponseDto, prefix: 'ba' | 'ess' | 'pcs' | 'dsl') {
  return [
    createValueRow('P ( kW)( A/R/P)', formatTriple([readField(source, `${prefix}AtpTot`), readField(source, `${prefix}RtpTot`), readField(source, `${prefix}ArpTot`)])),
    createValueRow('V ( V )(L12/L23/31)', formatTriple([readField(source, `${prefix}PtpvL12`), readField(source, `${prefix}PtpvL23`), readField(source, `${prefix}PtpvL31`)])),
    createValueRow('A ( A )(L1/L2/L3)', formatTriple([readField(source, `${prefix}PaL1`), readField(source, `${prefix}PaL2`), readField(source, `${prefix}PaL3`)])),
    createValueRow('FR ( Hz )(L1/L2/L3)', formatTriple([readField(source, `${prefix}PfrL1`), readField(source, `${prefix}PfrL2`), readField(source, `${prefix}PfrL3`)]))
  ];
}
...
function createBank(id: string, name: string, currentKw: NumberLike, accumulatedKw: NumberLike, powerFactor: NumberLike) {
  return {
    ...
    name,
    rows: [
      { label: 'kW', status: formatNumber(currentKw), dAccm: formatNumber(accumulatedKw) },
      { label: 'PF', status: formatPowerFactor(powerFactor), dAccm: formatPercent(powerFactor) }
    ]
  };
}
